...
module SplitIoClient
  module Engine
    class PushManager
      def initialize(config, sse_handler, api_key, telemetry_runtime_producer, request_decorator)
        @config = config
        @sse_handler = sse_handler
        @auth_api_client = AuthApiClient.new(@config, telemetry_runtime_producer, request_decorator)
        @api_key = api_key
