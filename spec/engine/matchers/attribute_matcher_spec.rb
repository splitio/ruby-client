...
describe SplitIoClient::AttributeMatcher do
  describe 'equal to matcher' do
    ...
    context 'with attribute' do
      ...
      it 'matches' do
        expect(described_class.new('attr', matcher).match?('key', nil, nil, attr: 'value')).to eq(true)
      end

      it 'does not match' do
        expect(described_class.new('attr', matcher).match?('key', nil, nil, attr: 'vvalue')).to eq(false)
      end
    end
  end
  ...
  describe 'ends with matcher' do
    ...
    context 'with attribute' do
      ...
      it 'does not match' do
        expect(described_class.new('attr', matcher).match?('key', nil, nil, attr: 'valuee')).to eq(false)
      end
    end
  end
end
