...
describe SplitIoClient::ContainsAnyMatcher do
  ...
  it 'matches' do
    expect(described_class.new('attr', remote_array, @split_logger).match?(attributes: { attr: %w[a] }))
      .to be(true)
    expect(described_class.new('attr', remote_array, @split_logger).match?(attributes: { attr: %w[a d c] }))
      .to be(true)
    expect(described_class.new('attr', remote_array, @split_logger).match?(attributes: { attr: %w[c d e] }))
      .to be(true)
  end

  it 'does not match' do
    expect(described_class.new('attr', %w[], @split_logger).match?(attributes: { attr: %w[a b c] }))
      .to be(false)
