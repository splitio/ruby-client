...
describe SplitIoClient::PartOfSetMatcher do
  ...
  it 'matches' do
    ...
    expect(described_class.new('attr', remote_array).match?(nil, nil, attr: %w(a b c))).to be(true)
    expect(described_class.new('attr', remote_array).match?(nil, nil, 'attr' => %w(a b c))).to be(true)
    expect(described_class.new(:attr, remote_array).match?(nil, nil, attr: %w(a b c))).to be(true)
    expect(described_class.new(:attr, remote_array).match?(nil, nil, 'attr' => %w(a b c))).to be(true)

    expect(described_class.new('attr', remote_array).match?(nil, nil, attr: %w(a b))).to be(true)
    expect(described_class.new('attr', remote_array).match?(nil, nil, attr: %w(a))).to be(true)
  end

  it 'does not match' do
    expect(described_class.new('attr', remote_array).match?(nil, nil, attr: %w(a b c d))).to be(false)
