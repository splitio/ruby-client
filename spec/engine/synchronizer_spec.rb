...
describe SplitIoClient::Engine::Synchronizer do
  ...
  let(:synchronizer) do
    ...
    parameters = {
      split_fetcher: SplitIoClient::Cache::Fetchers::SplitFetcher.new(splits_repository, rule_based_segments_repository, api_key, config, runtime_producer),
  ...
  it 'sync_all synchronous - should return false' do
    stub_request(:get, 'https://sdk.split.io/api/splitChanges?s=1.3&since=-1&rbSince=-1').to_return(status: 500)
  ...
  it 'fetch_splits' do
    ...
    synchronizer.fetch_splits(1506703262916, 0)
    expect(a_request(:get, 'https://sdk.split.io/api/splitChanges?s=1.3&since=-1&rbSince=-1')).to have_been_made.once
  end
  ...
  it 'fetch_splits - with CDN bypassed' do
    ...
    stub_request(:get, 'https://sdk.split.io/api/splitChanges?s=1.3&since=1506703262918&rbSince=-1')
      .to_return(status: 200, body:
    '{"ff":{
      "d": [],
      "s": 1506703262918,
    ...
    stub_request(:get, 'https://sdk.split.io/api/splitChanges?s=1.3&since=1506703262918&rbSince=-1&till=1506703262920')
      .to_return(status: 200, body:
    '{"ff":{
      "d": [],
      "s": 1506703262918,
    ...
    synchronizer.fetch_splits(1_506_703_262_920, 0)

    expect(a_request(:get, 'https://sdk.split.io/api/splitChanges?s=1.3&since=-1&rbSince=-1')).to have_been_made.once
    expect(a_request(:get, 'https://sdk.split.io/api/splitChanges?s=1.3&since=1506703262918&rbSince=-1')).to have_been_made.times(9)
    expect(a_request(:get, 'https://sdk.split.io/api/splitChanges?s=1.3&since=1506703262918&rbSince=-1&till=1506703262920')).to have_been_made.once
  end
